...
@section('content')
    <div class="content">
        ...
        <div class="row">
            <div class="col-xl-4 d-flex">
                            ...
                            <h6 class="fw-medium d-flex align-items-center justify-content-center mb-4">
                                ...
                                <span id="punchText">
                                    ...
                                    @elseif(!$attendance->punch_out)
                                        Punch In at {{ \Carbon\Carbon::parse($attendance->punch_in)->format('h:i A') }}
                                    @else
                                        Completed
                                    @endif
                                </span>
                ...
                <input type="hidden" id="punch_in" value="{{ $attendance->punch_in ?? '' }}">
                <input type="hidden" id="punch_out" value="{{ $attendance->punch_out ?? '' }}">
            </div>
    ...
    <script>
        ...
        $('#punchBtn').click(function() {
            ...
            if (type === 'out') {
                ...
                let punchInTime = "{{ $attendance->punch_in ?? '' }}";
    ...
    <script>
        $(document).ready(function() {
            ...
            $('#leave_type_id').change(function() {
                ...
                $.ajax({
                    url: "{{ route('employee.leave.details') }}",
                    type: "GET",
                    data: {
    ...
    <Script>
        ...
        $(document).on('click', '.mark-read', function() {
            ...
            $.ajax({
                url: "{{ route('employee.leave.mark.read') }}",
                type: "POST",
                data: {
                    _token: "{{ csrf_token() }}",
                    id: id
